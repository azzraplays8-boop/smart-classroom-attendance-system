...
function normalizeStudentNumber(v) {
...
function buildStudentName({ lastName, firstName, middleName }) {
...
function validateForm({ values, existingStudentNumbers, ignoreStudentNumber }) {
  const errors = {};
  ...
  const sn = normalizeStudentNumber(values.studentNumber);
  if (
    sn &&
...
function AddStudentModal({
  ...
  const existingStudentNumbers = useMemo(() => {
    ...
    for (const s of existingStudents) {
      if (s?.studentNumber != null) {
        const v = String(s.studentNumber).trim().toLowerCase();
        if (v) set.add(v);
      }
    }
    return set;
  ...
  const computedErrors = useMemo(() => {
    return validateForm({
      ...
      ignoreStudentNumber: editMode ? originalStudentNumber : undefined,
    });
  }, [values, existingStudentNumbers, editMode, originalStudentNumber]);
  ...
  useEffect(() => {
    ...
    // When switching between students, the parent updates `initialStudent`.
    ...
    if (editMode && initialStudent) {
      ...
      const asForm = {
        studentNumber: String(initialStudent.studentNumber ?? ""),
        lastName: String(initialStudent.lastName ?? ""),
        firstName: String(initialStudent.firstName ?? ""),
        middleName: String(initialStudent.middleName ?? ""),
        gender: String(initialStudent.gender ?? ""),
        dateOfBirth: toISODate(initialStudent.dateOfBirth),
        course: String(initialStudent.course ?? ""),
        yearLevel: normalizeYearLevel(initialStudent.yearLevel ?? initialStudent.year ?? ""),
        section: String(initialStudent.section ?? ""),
        email: String(initialStudent.email ?? ""),
        contactNumber: String(initialStudent.contactNumber ?? ""),
      };

      // Backward compatibility: if some older payloads use a combined `name`, derive fields.
      if ((!asForm.lastName || !asForm.firstName) && typeof initialStudent.name === "string") {
        const parts = initialStudent.name.split(",");
      ...
      setValues(asForm);
      setOriginalStudentNumber(String(asForm.studentNumber || ""));
      setErrors({});
      setTouched({});
      return;
    }
    ...
    setOriginalStudentNumber("");
    setErrors({});
    setTouched({});
  }, [isOpen, editMode, initialStudent, initialValues]);
  ...
  const handleSubmit = async () => {
    ...
    const newStudent = {
      ...
      studentNumber: normalizeStudentNumber(values.studentNumber),
      ...
      photo: initialStudent?.photo || null,
      photoFile: photoFile,
    ...
    const didSave = await onSave?.(newStudent);
            ...
            <div className="sis-form">
              ...
              >
                ...
                <div style={{ padding: "14px 18px 18px", borderBottom: "1px solid rgba(226,232,240,.95)", display: "flex", alignItems: "center", gap: 20 }}>
                  {(photoPreview || (editMode && initialStudent?.photo && !removePhoto && !photoPreview)) ? (
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                    ...
                    <div style={{ display: "flex", gap: 8 }}>
                      ...
                      >
                        {editMode && initialStudent?.photo && !removePhoto && !photoPreview ? "Change Photo" : "Upload Photo"}
                      </button>
                      {(editMode && initialStudent?.photo && !removePhoto && !photoPreview) || photoPreview ? (
                        <button
                          type="button"
                          className="sis-btn sis-btn-outline"
                ...
                <div className="sis-grid sis-grid-3">
                  <div className="sis-field">
                    ...
                    <input
                      ...
                      onBlur={markTouched("studentNumber")}
                      aria-invalid={Boolean(errors.studentNumber)}
                      aria-describedby={
                        visibleError("studentNumber") && errors.studentNumber
                          ? "studentNumber-error"
                          : undefined
                      }
                    />
                    {visibleError("studentNumber") && errors.studentNumber ? (
                      <div className="sis-error" id="studentNumber-error">
