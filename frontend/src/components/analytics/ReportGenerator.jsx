...
function ReportGenerator({ records, organizationSettings, labels }) {
  ...
  const handleExportPDF = () => {
    ...
    try {
      ...
      const summary = {
        ...
        present: filteredRecords.filter((r) => String(r.status || "").toLowerCase() === "present").length,
        late: filteredRecords.filter((r) => String(r.status || "").toLowerCase() === "late").length,
        absent: filteredRecords.filter((r) => String(r.status || "").toLowerCase() === "absent").length,
        excused: filteredRecords.filter((r) => String(r.status || "").toLowerCase() === "excused").length,
      };
      ...
      doc.setFontSize(18);
      doc.setFont("helvetica", "bold");
      doc.text("KATAGA", 16, 16);
      doc.setFontSize(9);
      doc.setFont("helvetica", "normal");
      doc.text("Kapatiran ng Talino at Galing", 16, 23);
      doc.text(`Generated On: ${generatedAt.toLocaleString()}`, pageWidth - 100, 16);

      doc.setTextColor(15, 23, 42);
      doc.setFontSize(15);
      doc.setFont("helvetica", "bold");
      doc.text("ATTENDANCE REPORT", 16, 42);
      doc.setFontSize(9);
      doc.setFont("helvetica", "normal");
      doc.text(`Report Period: ${getFiltersLabel()}`, 16, 52);
      doc.text(`Summary: Total ${summary.total} | Present ${summary.present} | Late ${summary.late} | Absent ${summary.absent} | Excused ${summary.excused}`, 16, 60);
      ...
      autoTable(doc, {
        ...
        didDrawPage: () => {
          ...
          doc.text(`Page ${doc.internal.getNumberOfPages()}`, pageWidth - 22, pageHeight - 10);
        },
      });
  ...
  const handleExportExcel = () => {
    ...
    try {
      ...
      ws["!cols"] = [{ wch: 14 }, { wch: 18 }, { wch: 24 }, { wch: 18 }, { wch: 12 }, { wch: 12 }, { wch: 12 }, { wch: 14 }];
