...
/**
 * Horizontal bar chart of participant count per section/group.
 */
function SectionChart({ records }) {
  const data = useMemo(() => {
    const map = new Map();
    for (const record of records || []) {
      const section = String(record.section || record.group_name || "Unknown").trim() || "Unknown";
      if (!map.has(section)) {
        map.set(section, { section, present: 0, late: 0, absent: 0, total: 0 });
      }
  ...
  return (
    <div className="an-card">
      <div className="an-card-header">
        <div>
          <h3 className="an-card-title">Attendance by Section</h3>
          <p className="an-card-subtitle">Participant count per section / group</p>
        </div>
      </div>
